...
function processSum(numberList, callback) {
  ...
  // return callback(a + b);
  // }, 0);
  const numList = numberList.reduce((count, b) => {
    return count + b;
  }, 0);

  return callback(numList);
}
...
function processDuplicateFree(list, callback) {
  ...
  const noDups = list.filter((item, index) => list.indexOf(item) === index);
...
function firstNamesAllCaps(runners) {
  const fNames = runners.map(function(fname) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const mySize = runners.filter(function(num) {
    return num.shirt_size === tShirtSize;
  });
  return mySize;
  /* CODE HERE */
}
...
function tallyUpDonations(runners) {
  const donations = runners.reduce((acc, CurrValue) => {
    return acc + CurrValue.donation;
  }, 0);
